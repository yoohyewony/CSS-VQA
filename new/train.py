            ...
            if mode=='updn':
                for i in range(model_num):
                    ...
                    if(i==0):
                        ...
                        logit_sum += pred
                        '''
                        tmp, logit = compute_score_with_logits(pred, a.data)
                        logit = logit*(logit==a)
                        score = torch.logical_or(tmp, score)
                        '''
                    #loss = F.binary_cross_entropy_with_logits(pred, a) + F.binary_cross_entropy_with_logits(logit, a)
                    loss += 0.5*F.binary_cross_entropy_with_logits(logit, a, reduce=False).sum(1)
                    
                loss += F.binary_cross_entropy_with_logits(logit_sum, a, reduce=False).sum(1)
                print(loss.shape)
                loss *= a.size(1)
                print(loss)
                loss.backward()
    ...
    for v, q, a, b, qids, _ in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
        ...
        for i in range(model_num):
            ...
            if(i==0):
                ...
                logit_sum = pred
            '''
            t, _ = compute_score_with_logits(pred, a.cuda())
            accuracy[i] += t.cpu().numpy().sum()
            if(i==0):
                logit, _ = compute_score_with_logits(pred, a.cuda())
                #scores = logit.sum().view(1)
            else:
                tmp, _ = compute_score_with_logits(pred, a.cuda())
        ...
        #batch_score = logit.cpu().numpy().sum(1)
        batch_score = compute_score_with_logits(logit_sum, a.cuda()).cpu().numpy().sum(1)
